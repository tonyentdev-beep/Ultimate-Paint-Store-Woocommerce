...
const SheensManager = ({ sheens, fetchSheens }) => {
    ...
    const handleSave = async () => {
        ...
        try {
            if (editingId) {
                await apiFetch({ path: `/paint-store/v1/sheens/${editingId}`, method: 'PUT', data: { name, description } });
            } else {
                await apiFetch({ path: '/paint-store/v1/sheens', method: 'POST', data: { name, description } });
            }
            setName(''); setDescription(''); setEditingId(null); fetchSheens();
        } catch (error) {
